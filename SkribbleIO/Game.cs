                ...
                while (true)
                {
                    try
                    {
                        using (MemoryStream ms = new MemoryStream())
                        {
                            ...
                            // Lit tant que des donn�es arrivent
                            while ((bytesRead = stream.Read(buffer, 0, buffer.Length)) > 0)
                            {
                                ms.Write(buffer, 0, bytesRead);
                                // Ajoute une pause ou logique pour d�tecter fin d'image
                            ...
                            //        pictureBox1.Image = image; // affiche le dessin re�u
                            //    }));
                            //}
                        }
                    }
                    ...
                    {
                        Console.WriteLine("Erreur r�ception : " + ex.Message);
                        break;
                    }
                }
            });
